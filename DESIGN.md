...
- **Organic Hand-Drawn Outlines:** Authentic PaperCSS bounding curves (`255px 15px 225px 15px/15px 225px 15px 255px`) with inked 2px solid `#212121` strokes.
- **Editorial Broadside Hierarchy:** Bold condensed newsprint headings paired with casual hand-drawn body notes and monospace telemetry data.
- **Mechanical Drop Shadows:** Zero blurred gaussian drop-shadows; all depth is rendered through hard-edged inked offsets (`4px 4px 0px #000`).
...
### Named Rules
**The Inked Boundary Rule.** Every surface, input, button, and badge must have a visible inked border (`2px solid #212121` or `1.5px solid #212121`). Borderless floating cards are prohibited.
...
### Hierarchy
- **Display** (weight 900, `clamp(2rem, 5vw, 3rem)`, line-height 1.1): Hero banner titles, major page headlines, and gazette mastheads.
- **Headline** (weight 700, `1.25rem` / `20px`, line-height 1.25): Section headers, modal titles, and student names.
- **Title** (weight 700, `1rem` / `16px`, line-height 1.3): Project card titles, repository names, and navigation links.
- **Body** (weight 400, `0.875rem` / `14px`, line-height 1.5): Bio paragraphs, project descriptions, and general copy.
- **Label / Telemetry** (weight 700, `0.75rem` / `12px`, uppercase, letter-spacing 0.05em): GitHub usernames, star counts, tags, dates, and commit heatmaps.
...
- **Central Paper Container:** Max-width bounded container (`max-w-6xl`) centered on the canvas with generous responsive padding (`p-4` to `p-8`).
- **Tactile Column Grids:** 2-column or 3-column responsive grids (`grid-cols-1 md:grid-cols-2 lg:grid-cols-3`) with consistent gap rhythm (`gap-5` / `20px`).
- **Dashed Newsprint Dividers:** Visual sections are partitioned by dashed ink borders (`border-b-2 border-dashed border-[#212121]`) rather than solid gray hairline dividers.
...
### Shadow Vocabulary
- **Sheet Elevation** (`box-shadow: 6px 6px 0px rgba(0, 0, 0, 0.85)`): Main central paper page wrapper.
- **Card Elevation** (`box-shadow: 4px 4px 0px rgba(0, 0, 0, 0.85)`): Repository cards, student profile headers, and search bars.
- **Button Rest** (`box-shadow: 3px 3px 0px #212121`): Interactive paper buttons.
- **Button Hover** (`box-shadow: 5px 5px 0px #212121`, `transform: translate(-1px, -1px)`): Tactile raised hover state.
- **Button Active / Press** (`box-shadow: 1px 1px 0px #212121`, `transform: translate(2px, 2px)`): Inward mechanical press.

### Named Rules
**The Hard-Edge Shadow Rule.** Never use `blur` radii on box shadows. All shadows must be offset hard-edge blocks (`Xpx Ypx 0px #212121` or `rgba(0,0,0,0.85)`).

## Shapes

- **Asymmetric Paper Curve:** The foundational shape is PaperCSS's signature 4-corner organic radius: `border-radius: 255px 15px 225px 15px/15px 225px 15px 255px`.
- **Inked Stroke Weight:** 2px solid `#212121` on primary components; 1.5px on compact badges; 2px dashed on partition dividers.
- **Paper Tape Effect:** Accent headers and pinned badges can optionally utilize the pseudo-element tape accent (`.paper-tape`) with a translucent parchment strip (`background-color: rgba(235, 230, 210, 0.75)`).

## Components

### Buttons
- **Shape:** Organic paper curve (`255px 15px 225px 15px/15px 225px 15px 255px`).
- **Primary / Dark:** Background `#212121`, text `#FEFCF6`, uppercase `Neucha` bold, padding `0.5rem 1.1rem`.
- **Secondary / Standard:** Background `#FEFCF6`, text `#212121`, border `2px solid #212121`, shadow `3px 3px 0px #212121`.
- **Hover / Active:** Smooth `0.12s ease-in-out` translation with dynamic shadow depth.

### Cards / Containers
- **Corner Style:** Organic paper curve with 2px solid `#212121` border.
- **Background:** `#FEFCF6` for standard cards; `#FAF6EC` for featured or secondary highlighted cards.
- **Shadow:** `4px 4px 0px rgba(0, 0, 0, 0.85)`, transitioning to `6px 7px 0px` on hover with `-2px` Y-translation.

### Badges & Chips
- **Style:** Compact pill with organic PaperCSS radius, `1.5px solid #212121`, padding `0.1rem 0.55rem`.
- **Palette:** `#EFE9DB` (neutral), `#FDE68A` (lead/pinned), `#D1FAE5` (published), `#212121` (step indicator).

### Inputs & Fields
- **Style:** Background `#FFFFFF`, border `2px solid #212121`, organic paper curve, shadow `2px 2px 0px rgba(0,0,0,0.4)`.
- **Focus:** Outline none, shadow expansion to `4px 4px 0px #212121`.

### Commit Heatmap Grid
- **Style:** 52-week horizontal calendar with 7-day columns (`11px` rounded `1px` tiles), custom tooltip cards, and 5-level green ink fill levels.

## Do's and Don'ts

### Do:
- **Do** wrap every card, modal, and interactive surface with `.paper-card` or `.paper-sheet` and a `2px solid #212121` border.
- **Do** use hard-edge drop shadows (`Xpx Ypx 0px #212121`) for all interactive and elevated elements.
- **Do** use dashed dividers (`border-b-2 border-dashed border-[#212121]`) between article sections and cards.
- **Do** format dates and headlines in classic broadsheet style ("WEDNESDAY, 26TH AUGUST 2026", uppercase headings).

### Don't:
- **Don't** use standard Tailwind geometric rounded corners (e.g. `rounded-xl`, `rounded-2xl`) without the PaperCSS asymmetric radius.
